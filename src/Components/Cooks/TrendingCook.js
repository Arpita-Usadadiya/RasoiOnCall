...
const TrendingCooks = () => {
  ...
  useEffect(() => {
    const fetchChefs = async () => {
      try {
        const res = await fetch("http://localhost:8000/chef/getAll");
  ...
  if (loading) {
    return <p className="text-center py-10">Loading chefs...</p>;
  }

  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <h2 className="text-lg font-semibold mb-4">Trending cooks</h2>
        ...
        {chefs.map((chef) => (
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <img
                src={chef.profilepic || "https://dummyimage.com/80x80/eee/000"}
                alt={chef.name}
      ...
      {/* App Download Section (like screenshot) */}
